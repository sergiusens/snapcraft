...
class Config:
    ...
    def __init__(self, project: project.Project) -> None:
        ...
        snapcraft_yaml = dict(project.info.raw_snapcraft_yaml)
    ...
    def stage_env(self):
        stage_dir = self._project.stage_dir
        env = []

        env += runtime_env(stage_dir, self._project.arch_triplet)
    ...
    def snap_env(self):
        prime_dir = self._project.prime_dir
        env = []

        env += runtime_env(prime_dir, self._project.arch_triplet)
    ...
    def project_env(self):
        return [
            ...
            snapcraft_global_environment(self._project).items()
        ]
    ...
    def _expand_env(self, snapcraft_yaml):
        ...
        for key in snapcraft_yaml:
            ...
            replacements = environment_to_replacements(
                snapcraft_global_environment(self._project))
