...
class Application:
    ...
    def _is_adapter(self, adapter: project_loader.Adapter) -> bool:
        adapter_value = self._app_properties.get("adapter")
        if adapter_value is None:
            return False

        return project_loader.Adapter[adapter_value.upper()] == adapter
    ...
    def _fix_sockets(self) -> None:
        ...
        sockets = self._app_properties.get("sockets", {})
    ...
    def get_yaml(
        self, *, prepend_command_chain: Optional[Sequence[str]] = None
    ) -> OrderedDict:
        """Returns and ordered dicitonary with the transformed app entry."""
