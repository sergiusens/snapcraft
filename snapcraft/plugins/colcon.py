...
class ColconPlugin(snapcraft.BasePlugin):
        ...
        return textwrap.dedent(
            ...
            """.format(
                ...
                codename=_BASE_TO_UBUNTU_RELEASE_MAP[
                    self.project.info.get_effective_base()
                ],
            )
        )
    ...
    def __init__(self, name, options, project):
        ...
        if project.info.get_effective_base() != _ROSDISTRO_TO_BASE_MAP[self._rosdistro]:
            raise ColconPluginBaseError(
                self.name, project.info.get_effective_base(), self._rosdistro
            )
