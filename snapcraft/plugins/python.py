...
class PythonPlugin(snapcraft.BasePlugin):
    ...
    def plugin_stage_packages(self):
        ...
        if self.project.info.get_effective_base() in ("core", "core16", "core18"):
            stage_packages = [python_base]
        else:
            stage_packages = []

        if (
            self.project.info.get_effective_base() == "core18"
            and python_base == "python3"
        ):
    ...
    def __init__(self, name, options, project):
        ...
        self._setup_base_tools(project.info.get_effective_base())
